...
if st.button("🚀 GÉNÉRER LE RAPPORT D'ANALYSE"):
    try:
        ...
        pnb_moy, rn_moy = pnb_tot / 4, rn_tot / 4
        ...
        def diag_rentabilite(sect, moy):
            s = sect.lower()
            limites = [20, 30] if "financier" in s else ([5, 15] if "telecom" in s else ([5, 12] if "consommation" in s else [5, 10]))
        ...
        st.subheader("3. Historique Dividendes & Distribution")
        rows = []
        for i in range(len(annees)):
            note = "⚠️ ALERTE : TD > 100%" if tds[i] > 100 else ("💎 STRATÉGIE RENTE" if tds[i] > 50 else "📈 CROISSANCE")
            rows.append({"Année": annees[i], "Marge Nette (%)": f"{marges[i]:.2f} %", "TD (%)": f"{tds[i]:.2f} %", "Observation": note})
        st.table(rows)
        ...
        v2.metric("VMC", f"{vmc:.0f} FCFA")
        v3.metric("PBR", f"{pbr:.2f}", evaluer(pbr, [0.7, 1]), delta_color="inverse")
        v4.metric("RVC", f"{rvc:.2f}", evaluer(rvc, [7, 10]), delta_color="inverse")
...
# --- 5. SECTION DISCLAIMER (FIXE EN BAS) ---
st.markdown("<br><br>", unsafe_allow_html=True)
with st.expander("⚠️ CLAUSE DE NON-RESPONSABILITÉ (DISCLAIMER)"):
    st.write("""
    Cet outil est une application de calcul financier à but éducatif et informatif uniquement. 
    Les analyses générées par cette plateforme ne constituent en aucun cas des conseils en investissement, 
    des recommandations d'achat ou de vente de titres boursiers. 
    
    L'investissement sur les marchés financiers présente des risques de perte en capital. 
    L'auteur de cette application ne saurait être tenu responsable des décisions prises sur la base 
    des calculs fournis. Il est fortement recommandé de consulter un Conseiller en Investissement Financier (SGI) 
    agréé par l'AMF-UMOA avant toute transaction sur la BRVM.
    """)
